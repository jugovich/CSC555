...
for line in sys.stdin:
    try:
        ...
        if curr_key != key:
            ...
            requests.get('http://54.148.144.207:8983/solr/admin/cores?action=CREATE&name=user_%s&instanceDir=collection1&config=solrconfig.xml&schema=schema.xml&dataDir=data' % key)
            #connect to the solr collection
            solr = pysolr.Solr('http://54.148.144.207:8983/solr/user_%s/' % key, timeout=30)
